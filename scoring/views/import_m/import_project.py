...
def import_project(sheet):
    ...
    #SAVE PROJECT
        p = Project(project_id, description, project_title, project_category, c.float1(avg_score), c.int1(rank), c.float1(z_score), c.float1(scaled_score), c.float1(scaled_rank), c.float1(scaled_z), c.float1(isef_score), c.int1(isef_rank))
        p.save()
